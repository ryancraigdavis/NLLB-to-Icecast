...
class AudioCapture:
    ...
    def get_optimal_device(self) -> Optional[int]:
        ...
        professional_keywords = ['focusrite', 'behringer', 'presonus', 'motu', 'rme', 'zoom', 'tascam']
    ...
    def _process_audio(self):
        ...
        processing_interval = 5.0  # Process every 5 seconds for real-time translation
        ...
        while self.is_recording:
            try:
                ...
                # Check if it's time to process (every 5 seconds)
                current_time = time.time()
                if (current_time - last_processing_time >= processing_interval and 
                    len(self.rolling_buffer) >= self.sample_rate * 10):  # At least 10 seconds of audio
                    ...
                    if self.transcription_callback:
                        ...
                        logger.debug(f"Sent {len(self.rolling_buffer)/self.sample_rate:.1f}s of audio for processing")
    ...
    def save_buffer_to_file(self, filename: str):
        ...
        logger.info(f"Saved {len(self.rolling_buffer)/self.sample_rate:.1f} seconds to {filename}")
...
def test_capture():
    ...
    try:
        ...
        print("Speak into your microphone. The system will process 30-second rolling buffers.")
